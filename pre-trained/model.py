...
parser=argparse.ArgumentParser(description='call all scores and compute the visual context based re-ranker')
...
# visual confident based visual re-ranker
class Visual_re_ranker:
        def __init__(self, visual_context_prob, sim):
            self.visual_context_prob = visual_context_prob
            self.sim = sim
        def p_minus (self):
            score = pow(float(sim), float(visual_context_prob))
...
input_path= 'Visual_re-ranker.txt'
# re-ranked beam with visual context
output_path = 'Visual_re-rank_re-ranked_output.txt'
